...
#Load training data
full_data = pd.read_csv("./data/ELSA_full_trainingData.csv").values.tolist()
...
output_dir = './data/fivefold_val_flags(ELSA).csv'
output = pd.DataFrame(columns=column,data = val_flags)
output.to_csv(output_dir,index = None)


#Code 2:Set random seed
seed = 1000
random.seed(seed)
np.random.seed(seed)

#Load training data
full_data = pd.read_csv("./data/ELIM_full_trainingData.csv").values.tolist()
val_flags = np.zeros((len(full_data),5)).astype(int)

#Find all samples with IM label or EL label(no BA labels)
val_data_index_list = list()
for index,item in enumerate(full_data):
    EL = item[2]
    IM = item[3]
...
column = ['fold-1','fold-2','fold-3','fold-4','fold-5']
output_dir = './data/fivefold_val_flags(ELIM).csv'
output = pd.DataFrame(columns=column,data = val_flags)
output.to_csv(output_dir,index = None)
